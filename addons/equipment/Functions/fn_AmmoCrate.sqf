/*
Function name:ghostf_fnc_ghostf_fnc_AmmoCrate;
Author: Mr H.
...
Example(s):
[this,10,true,true] call ghostf_fnc_AmmoCrate;
*/
...
if (_isUnlimited)then {
    _box setVariable ["ghostf_AmmoMagazines", _allMagazinesType];

    _handlerIndex = _box addEventHandler
    ["ContainerOpened",
    {
    _box = _this select 0;
    _allMagazinesType = _box getVariable "ghostf_AmmoMagazines";



    clearMagazineCargoGlobal _box;
    {_box addMagazineCargoGlobal [_x,10];} forEach _allMagazinesType;
        _box addItemCargoGlobal ["ghostf_medbags_FirstAid",2];
        _box addItemCargoGlobal ["ghostf_medbags_MedicKit",4];
...
_box setVariable ["ghostf_AmmoCrateEHIndex", _handlerIndex, true];
...
_box setVariable ["ghostf_AmmoCrateParameters", _params, true];
...
_statement =
 ...
 _box = _this select 0;
 _params = _box getVariable "ghostf_AmmoCrateParameters";
 [[_params],
  {
  params ["_params"];
  _box = _params select 0;
  _handlerIndex = _box getVariable "ghostf_AmmoCrateEHIndex";
  ...
  _params call ghostf_fnc_AmmoCrate;
  }

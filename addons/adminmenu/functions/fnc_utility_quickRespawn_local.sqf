...
if (_oldUnitdata isEqualType []) then {
    ...
    // if (isClass(configFile >> "CfgPatches" >> "acre_core")) then  {
    //     [false] call acre_api_fnc_setSpectator;
    //     [{
    //         if (isNull player) exitWith {};
    //         if (isNil "ghostf_acre2_networksCreated") exitWith {}; //Ensure presets are created
    ...
    systemChat "[Ghosts of Battle Admin Menu] Quick Respawn failed: old unit data unavailable";
};

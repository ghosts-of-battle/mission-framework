...
private _isValidCurator = false;
{
    ...
    if (_isValidCurator) exitWith {
        ...
        "[Ghosts of Battle Admin Menu] You were given access to Zeus" remoteExec ["systemChat", _unit];
    };
} forEach allCurators;
...
if (!_isValidCurator) then {
    ...
    private _curator = (createGroup GVAR(sideCenter)) createUnit ["ModuleCurator_F", [0,0,5000], [], 0, "NONE"];

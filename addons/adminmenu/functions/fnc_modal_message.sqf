...
private _ctrlEditPos = [0.1 * ghostf_ADMINMENU_STD_WIDTH, 0.1 * ghostf_ADMINMENU_STD_HEIGHT, _ctrlGrpWidth - (0.2 * ghostf_ADMINMENU_STD_WIDTH), _ctrlGrpHeight - (1.3 * ghostf_ADMINMENU_STD_HEIGHT)];
...
_ctrlCombo ctrlSetPosition [0.1 * ghostf_ADMINMENU_STD_WIDTH, _ctrlGrpHeight - ghostf_ADMINMENU_STD_HEIGHT, _ctrlGrpWidth * 0.3, ghostf_ADMINMENU_STD_HEIGHT];
...
_ctrlButtonPreview ctrlSetPosition [(_ctrlGrpWidth * 0.7) - (0.2 * ghostf_ADMINMENU_STD_WIDTH), _ctrlGrpHeight - ghostf_ADMINMENU_STD_HEIGHT, (_ctrlGrpWidth * 0.15), ghostf_ADMINMENU_STD_HEIGHT];
...
_ctrlButtonPreview ctrlAddEventHandler ["buttonClick", {
    ...
    if (_editText isEqualTo "") then {
        systemChat "[Ghosts of Battle Admin Menu] Message can't be empty";
    } else {
        ...
        if (_venue isEqualTo QFUNC(showSubtitle)) then {
            ...
            if (_venue isEqualTo "hint") then {
                ...
                systemChat format ["[Ghosts of Battle Admin Message] %1", _editText];
            };
        };

        systemChat "[Ghosts of Battle Admin Menu] Message previewed";
    };
}];

private _ctrlButtonCommit = _display ctrlCreate [QGVAR(RscButtonMenu), -1, _ctrlGroup];
_ctrlButtonCommit ctrlSetPosition [(_ctrlGrpWidth * 0.85), _ctrlGrpHeight - ghostf_ADMINMENU_STD_HEIGHT, (_ctrlGrpWidth * 0.15) - (0.1 * ghostf_ADMINMENU_STD_WIDTH), ghostf_ADMINMENU_STD_HEIGHT];
...
_ctrlButtonCommit ctrlAddEventHandler ["buttonClick", {
    ...
    if (_editText isEqualTo "") then {
        ...
        if (_venue isEqualTo QFUNC(showSubtitle)) then {
            ...
            (format ["[Ghosts of Battle Admin Message] %1", _editText]) remoteExec [_venue, GVAR(utilityData)];
        };

        systemChat "[Ghosts of Battle Admin Menu] Message sent to player(s)";

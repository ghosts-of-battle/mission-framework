class Controls
{
    class BackgroundMessagesList: RscText
    {
        x = 0;
        y = 0;
        w = QUOTE(ghostf_ADMINMENU_MSGS_W_LIST);
        h = QUOTE(ghostf_ADMINMENU_MSGS_H_LIST);
    ...
    {
        idc = IDC_ghostf_ADMINMENU_MSGS_LIST;
        ...
        rowHeight = QUOTE(ghostf_ADMINMENU_STD_HEIGHT);
        colorText[] = {1,1,1,1};
    ...
    {
        idc = IDC_ghostf_ADMINMENU_MSGS_COPY;
        ...
        x = QUOTE(0 * ghostf_ADMINMENU_STD_WIDTH);
        y = QUOTE(19.5 * ghostf_ADMINMENU_STD_HEIGHT);
        w = QUOTE(9 * ghostf_ADMINMENU_STD_WIDTH);
    };

    class ButtonPrintAll: GVAR(RscButtonMenu)
    {
        idc = IDC_ghostf_ADMINMENU_MSGS_COPYALL;
        text = "PRINT ALL TO RPT";
        tooltip = "Prints all logged entries to RPT.";
        onButtonClick = QUOTE(_this call FUNC(messageLog_printAll));
        colorBackground[] = {0, 0, 0, 1};
        x = QUOTE(9.1 * ghostf_ADMINMENU_STD_WIDTH);
        y = QUOTE(19.5 * ghostf_ADMINMENU_STD_HEIGHT);
        w = QUOTE(7 * ghostf_ADMINMENU_STD_WIDTH);
    };
};

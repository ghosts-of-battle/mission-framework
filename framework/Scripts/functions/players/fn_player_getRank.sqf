 ...
 *
 * Example:
 * [player, 'BIS'] call Ghost_fnc_player_getRank
 *
 */
...
if (_rankType == 'BIS') then {
    switch (_rankType == 'BIS') do {
        ...
        case (_playerUID in (EGVAR(Settings,ranks_corporal))) : {_return = 'CORPORAL'};
        case (_playerUID in (EGVAR(Settings,ranks_sergeant))) : {_return = 'SERGEANT'};
        case (_playerUID in (EGVAR(Settings,ranks_lieutenant))) : {_return = 'LIEUTENANT'};
        case (_playerUID in (EGVAR(Settings,ranks_captain))) : {_return = 'CAPTAIN'};
        case (_playerUID in (EGVAR(Settings,ranks_major))) : {_return = 'MAJOR'};
        case (_playerUID in (EGVAR(Settings,ranks_colonel))) : {_return = 'COLONEL'};
